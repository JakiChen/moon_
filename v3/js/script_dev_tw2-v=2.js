...
var beforeOpenScrollPos = 0;
 ...
 function firstrun(){
      ...
      $.ajax({
            url : '/naminsik/v3/js/pfdata.json?v=' + (new Date).getTime(),
            dataType : "json",
            // async : false,
       ...
       for (var i = 0; i < portfolioData.length; i++){
            ...
            .find('.btn.detailview').on('click', function(event){
                  ...
                  if(!$mainnav.hasClass('detailpageOpen')){
                        ...
                        // var  getWebm = (portfolioData[getIdx].movie.mp4) ? ' <source src="' + portfolioData[getIdx].movie.webm+ '?'+ (new Date).getTime() + '" type="video/webm" />'  : '';
                        // if(portfolioData[getIdx].movie.mp4.length > 0){
                        //       insertHtml += '<li><div class="loadanimate" id="videoload"><div></div><div></div><div></div><div></div></div><video autoplay loop muted playsinline oncanplaythrough="videoReady(true)" ><source src="' + portfolioData[getIdx].movie.mp4 + '?'+ (new Date).getTime() + '" onerror="fallback(parentNode)">' + getWebm + '<img src="' + portfolioData[getIdx].keyvisual[0] + '"></video></li>';
                        // }else{
                        ...
                        // .hover(function(){
                        //       closebtnMove = false;
                        //       $detailpage_close_btn.hide();
                        // },
                        // function(){
                        //       closebtnMove = true;
                        //       $detailpage_close_btn.show();
                        // })
                        // .viSimpleSlider({
                        // 			ease : 'easeInOutQuart',
                        // 			autoPlay : true,
                        ...
                        //                   desktopSwipe : false
                        // 		});
                        // $detailContent.find('.captureImg').html('<img src="' + portfolioData[getIdx].keyvisual + '" alt="' + portfolioData[getIdx].project + '">');
